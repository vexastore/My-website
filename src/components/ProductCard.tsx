...
export const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (remainingStock > 0) addToCart(product);
  };
  ...
  return (
    ...
    <div onClick={() => setIsDetailsOpen(true)} className="group cursor-pointer bg-[#050505] text-white">
      <div className="relative overflow-hidden rounded-md border border-white/5 bg-[#101010] p-0 shadow-[0_18px_45px_rgba(0,0,0,0.55)]">
        ...
        {/* Card image area — real image if available, gradient placeholder otherwise */}
          ...
          <div className="grid grid-cols-1 gap-0 md:grid-cols-2">
            <div className="bg-stone-100 p-3">
              {selectedImage ? (
                ...
                <div className={`flex h-full min-h-[320px] w-full items-center justify-center rounded-xl bg-gradient-to-br ${gradientClass}`}>
              ...
              {productImages.length > 1 && (
                <div className="mt-3 grid grid-cols-5 gap-2">
                  {productImages.map((img, idx) => (
                    ...
                    >
                      <img src={img} alt={`${displayName} ${idx + 1}`} className="h-full w-full object-cover" />
                    </button>
                  ))}
                </div>
              )}
            ...
            <div className="space-y-5 p-5 sm:p-7">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="rounded-xl border border-purple-100 bg-purple-50 p-3 text-purple-800">
                  ...
                  <p className="text-sm font-black">{isArabic ? 'تغليف سري ودفع عند الاستلام' : 'Discreet packaging & cash on delivery'}</p>
                </div>
              </div>
              ...
              >
                ...
                {product.stock === 0
                  ...
                  : remainingStock === 0
                  ? (isArabic ? 'تمت إضافة كل الكمية المتاحة' : 'All available quantity added')
                  : (isArabic ? 'أضف المنتج للسلة' : 'Add product to cart')}
              </button>
            </div>
          </div>
        </div>
