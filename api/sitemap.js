...
function buildProductUrl(slug, lastmod, image, nameEn) {
  const loc = `https://vexatoys.com/product/${slug}`;
  const imageBlock = (image && !image.startsWith('data:') && image.startsWith('http'))
    ? `\n    <image:image>\n      <image:loc>${escXml(image)}</image:loc>\n      <image:title>${escXml((nameEn || slug) + ' | Vexa Store Lebanon')}</image:title>\n    </image:image>`
    : '';
  return `  <url>
    <loc>${loc}</loc>
    <lastmod>${lastmod || today()}</lastmod>
    <changefreq>weekly</changefreq>
    <priority>0.80</priority>${imageBlock}
...
export default async function handler(req, res) {
    ...
    if (response.ok) {
      ...
      if (docs.length > 0) {
        productUrls = docs.map(doc => {
          ...
          const lastmod      = (doc.updateTime || doc.createTime || '').slice(0, 10) || today();
          return buildProductUrl(slug, lastmod, image, nameEn);
        });
      }
    }
  ...
  if (productUrls.length === 0) {
    productUrls = FALLBACK_PRODUCT_SLUGS.map(s => buildProductUrl(s, today(), '', s));
  }
