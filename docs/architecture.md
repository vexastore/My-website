...
Checkout uses React-controlled, storefront-styled combobox/listbox popovers for country calling codes and Lebanese cities. The controls preserve the existing order payload values while providing browser-independent rendering, keyboard selection, focus restoration, outside-click dismissal, and bilingual labels.
