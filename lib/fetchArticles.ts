...
async function readArticles(): Promise<BlogPost[]> {
  ...
  if (!base || !key) {
    console.warn('[fetchArticles] Supabase env vars missing — returning empty list');
    return [];
  }
  try {
    ...
    const response = await fetch(endpoint, {
      headers: { apikey: key },
      next: { revalidate: 300 },   // cooperates with unstable_cache; avoids cache: 'no-store' conflict
    });
    if (!response.ok) {
      console.error(`[fetchArticles] Supabase returned ${response.status} — returning empty list`);
      return [];
    }
    ...
    console.error('[fetchArticles] fetch failed — returning empty list:', err);
    return [];
  }
}
